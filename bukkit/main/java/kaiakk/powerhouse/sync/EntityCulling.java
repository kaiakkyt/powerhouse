...
import kaiakk.powerhouse.async.EntityCalculator;
import kaiakk.powerhouse.async.EntitySnapshot;
...
public class EntityCulling {
	...
	private void tickMain() {
		...
		if (!immediateRemovals.isEmpty()) {
			...
			try { kaiakk.powerhouse.sync.DebugLog.debug("EntityCulling: scheduling immediate removals: " + toKillNow.size()); } catch (Throwable ignored) {}
				SchedulerHelper.runLater(plugin, new Runnable() {
				public void run() {
					int removed = 0;
					for (UUID id : toKillNow) {
						Entity e = kaiakk.powerhouse.sync.EntityLookup.getEntity(id);
						if (e == null) continue;
						if (e instanceof Player) continue;
						try {
					...
					try { kaiakk.powerhouse.sync.DebugLog.debug("EntityCulling: applied immediate removals, removed=" + removed); } catch (Throwable ignored) {}
				}
			}, 1L);
		...
		SchedulerHelper.runAsync(plugin, new Runnable() {
			public void run() {
				...
				try { kaiakk.powerhouse.sync.DebugLog.debug("EntityCulling: async culling candidates=" + (toRemove == null ? 0 : toRemove.size())); } catch (Throwable ignored) {}
				SchedulerHelper.runLater(plugin, new Runnable() {
					public void run() {
						int removed = 0;
						...
						try { kaiakk.powerhouse.sync.DebugLog.debug("EntityCulling: applied async removals, removed=" + removed); } catch (Throwable ignored) {}
					}
				}, 1L);
			}
		});
