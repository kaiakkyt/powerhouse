...
import kaiakk.powerhouse.async.Calculations;
...
public class AllOptimizations implements Listener {
    ...
    private kaiakk.powerhouse.sync.DistanceController distanceController;
    private kaiakk.powerhouse.async.DistanceCalculator distanceCalculator;
    private kaiakk.powerhouse.sync.EntityPusher entityPusher;
    private kaiakk.powerhouse.sync.HopperLimiter hopperLimiter;
    private org.bukkit.scheduler.BukkitTask arrowTask = null;
    private kaiakk.powerhouse.async.DynamicScaler dynamicScaler = null;
    ...
    private final java.util.concurrent.ConcurrentHashMap<kaiakk.powerhouse.async.BlockKey, java.util.concurrent.atomic.AtomicInteger> redstoneListenerCounts = new java.util.concurrent.ConcurrentHashMap<>();
    ...
    public double getAverageMspt() {
        try {
            if (kaiakk.powerhouse.sync.FoliaChecker.isFolia(plugin)) return -1.0;
    ...
    public String getMsptSource() {
        try {
            if (kaiakk.powerhouse.sync.FoliaChecker.isFolia(plugin)) return "folia-disabled";
    ...
    public double getMsptVelocity() {
        try {
            if (kaiakk.powerhouse.sync.FoliaChecker.isFolia(plugin)) return 0.0;
    ...
    public boolean isLocationCulled(Location loc) {
        try {
            ...
            return kaiakk.powerhouse.async.Calculations.isLocationCulled(loc);
        } catch (Throwable ignored) {}
        return false;
    }
    ...
    public void start() {
        ...
        try { RecentActionTracker.init(plugin); RecentActionTracker.getInstance().startCleanupTask();
            try { kaiakk.powerhouse.sync.BookLagger.init(plugin); } catch (Throwable ignored) {}
            try { kaiakk.powerhouse.sync.ParticleCulling.init(plugin); } catch (Throwable ignored) {}
            try { kaiakk.powerhouse.sync.ProjectileManager.init(plugin); } catch (Throwable ignored) {}
            try { kaiakk.powerhouse.sync.ExplosionListener.init(plugin); } catch (Throwable ignored) {}
            try { kaiakk.powerhouse.sync.PassivePhysicsManager.init(plugin); } catch (Throwable ignored) {}
            
            try { int secs = kaiakk.powerhouse.ConfigHelp.getInt("recent-action.window-seconds", 30); RecentActionTracker.getInstance().setRetentionWindowSeconds(secs); } catch (Throwable ignored) {}
        } catch (Throwable ignored) {}
        ...
        // Every tick: force-update any blocks we suppressed while throttling so they re-evaluate power state.
        // Use a small deferred nudge (configurable) and deep nudge neighbors to avoid stale circuits.
        try {
            final int nudgeDelay = kaiakk.powerhouse.ConfigHelp.getInt("redstone-culling.nudge-delay-ticks", 2);
            Bukkit.getScheduler().runTaskTimer(plugin, new Runnable() {
                @Override
                public void run() {
                    try {
                        final java.util.List<Location> toFix = kaiakk.powerhouse.async.Calculations.drainThrottledLocations();
                        if (toFix == null || toFix.isEmpty()) return;
                        // Schedule a deferred deep nudge so the server has time to recover from lag
                                ...
                                try {
                                    for (Location loc : toFix) {
                                        try {
                                            ...
                                            // Deep nudge: immediate Cartesian neighbors
                                            try {
                                            ...
                                            // Also nudge one block further out in each Cartesian direction to restart stalled lines
                                            try {
                                            ...
                                            // Far-nudge: also nudge two blocks down (handles quasi-connectivity like pistons)
                                            try {
                                                try {
                                                    Block down2 = b.getRelative(BlockFace.DOWN, 2);
                                                    if (down2 != null) {
                                                        try { down2.getState().update(true, true); } catch (Throwable ignored) {}
                                                        // Also nudge its immediate neighbors
                                                        for (BlockFace face : new BlockFace[] { BlockFace.NORTH, BlockFace.SOUTH, BlockFace.EAST, BlockFace.WEST }) {
                                            ...
                                            // If the block exposes a numeric power property (AnaloguePowerable), and it's still >0,
                                            // force a re-evaluation by triggering a physics update.
                                            try {
                                                ...
                                                if (bd != null) {
                                                    try {
                                                        ...
                                                        // ignore - block data has no power property
                                                    } catch (Throwable ignored) {}
                                                }
                                            } catch (Throwable ignored) {}
                                        } catch (Throwable ignored) {}
        ...
        try {
            dynamicScaler = new kaiakk.powerhouse.async.DynamicScaler(plugin, metricCollector, 40);
            dynamicScaler.start();
        } catch (Throwable ignored) {}
        distanceController = new kaiakk.powerhouse.sync.DistanceController();
        distanceCalculator = new kaiakk.powerhouse.async.DistanceCalculator(plugin, metricCollector, distanceController);
        ...
        entityPusher = new EntityPusher(plugin, baseEntityThreshold, 2.0, 5.0, new java.util.function.DoubleSupplier() {
            public double getAsDouble() { return kaiakk.powerhouse.sync.ScaleUtils.getSmoothedMspt(); }
        });
        entityPusher.start();
            ...
            if (dynamicScaler != null) {
                ...
                scalerSubscriber = new java.util.function.Consumer<Double>() {
                    public void accept(Double s) {
                        try {
                            double mView = kaiakk.powerhouse.sync.ScaleUtils.multiplierFromScale(s, 0.6, 1.5);
                            ...
                            double mEntity = kaiakk.powerhouse.sync.ScaleUtils.multiplierFromScale(s, 0.5, 2.0);
                            int thr = Math.max(1, (int) Math.round(baseEntity * mEntity));
                            try { entityPusher.setThreshold(thr); } catch (Throwable ignored) {}

                            double mHopper = kaiakk.powerhouse.sync.ScaleUtils.multiplierFromScale(s, 0.5, 2.0);
                            ...
                            double mArrow = kaiakk.powerhouse.sync.ScaleUtils.multiplierFromScale(s, (5.0/30.0), 2.0);
    ...
    public void onWorldUnload(WorldUnloadEvent ev) {
        ...
        try {
            ...
            try { kaiakk.powerhouse.async.Calculations.clearWorldData(w.getName()); } catch (Throwable ignored) {}
            try { kaiakk.powerhouse.sync.DebugLog.debug("Powerhouse: cleared caches for unloaded world: " + w.getName()); } catch (Throwable ignored) {}
        } catch (Throwable ignored) {}
    }
...
private void startItemMergingTask(final World world) {
                ...
                for (org.bukkit.Chunk chunk : world.getLoadedChunks()) {
                    ...
                    runAtLocation(fChunk.getBlock(0, 64, 0).getLocation(), new Runnable() {
                        public void run() {
                            ...
                            final List<kaiakk.powerhouse.sync.ItemSnapshot> itemSnaps = new ArrayList<>();
                            final Map<Integer, Item> indexToItem = new HashMap<>();
                            final List<kaiakk.powerhouse.sync.ExperienceOrbSnapshot> orbSnaps = new ArrayList<>();
                            ...
                            for (int i = 0; i < droppedItems.size(); i++) {
                                ...
                                kaiakk.powerhouse.sync.ItemSnapshot s = new kaiakk.powerhouse.sync.ItemSnapshot(i, loc.getX(), loc.getY(), loc.getZ(),
                            ...
                            for (int i = 0; i < droppedOrbs.size(); i++) {
                                ...
                                orbSnaps.add(new kaiakk.powerhouse.sync.ExperienceOrbSnapshot(i, loc.getX(), loc.getY(), loc.getZ(), o.getExperience()));
                            }

                            final List<kaiakk.powerhouse.sync.ItemSnapshot> finalItemSnaps = new ArrayList<>(itemSnaps);
                            final Map<Integer, Item> finalIndexToItem = new HashMap<>(indexToItem);
                            final List<kaiakk.powerhouse.sync.ExperienceOrbSnapshot> finalOrbSnaps = new ArrayList<>(orbSnaps);

                            SchedulerHelper.runAsync(plugin, new Runnable() {
                                public void run() {
                                    try {
                                        final Map<kaiakk.powerhouse.sync.ItemSnapshot, List<kaiakk.powerhouse.sync.ItemSnapshot>> snapResult = kaiakk.powerhouse.async.Calculations.scanItemMergeCandidatesSnapshots(finalItemSnaps);

                                        final Map<Item, List<Item>> mapped = new HashMap<>();
                                        if (snapResult != null && !snapResult.isEmpty()) {
                                            for (Map.Entry<kaiakk.powerhouse.sync.ItemSnapshot, List<kaiakk.powerhouse.sync.ItemSnapshot>> ent : snapResult.entrySet()) {
                                                Item key = finalIndexToItem.get(ent.getKey().originalIndex);
                                                if (key == null) continue;
                                                List<Item> merged = new ArrayList<>();
                                                for (kaiakk.powerhouse.sync.ItemSnapshot s : ent.getValue()) {
                                        ...
                                        final Map<kaiakk.powerhouse.sync.ExperienceOrbSnapshot, List<kaiakk.powerhouse.sync.ExperienceOrbSnapshot>> xpGroups = kaiakk.powerhouse.async.Calculations.scanXpMergeCandidatesSnapshots(finalOrbSnaps);
                                        ...
                                        SchedulerHelper.run(plugin, new Runnable() {
                                            public void run() {
                                                ...
                                                if (xpGroups != null && !xpGroups.isEmpty()) {
                                                    ...
                                                    for (Map.Entry<kaiakk.powerhouse.sync.ExperienceOrbSnapshot, List<kaiakk.powerhouse.sync.ExperienceOrbSnapshot>> e : xpGroups.entrySet()) {
                                                        ...
                                                        for (kaiakk.powerhouse.sync.ExperienceOrbSnapshot s : e.getValue()) {
    ...
    private void performItemMerges(Map<Item, List<Item>> mergeGroups) {
        ...
        if (mergedCount.get() > 0) {
            kaiakk.powerhouse.sync.DebugLog.debug("Merged " + mergedCount.get() + " items into " + mergeGroups.size() + " stacks");
        }
    }
    ...
    public void onBlockPhysics(BlockPhysicsEvent event) {
        ...
        if (isRedstoneBlock(type)) {
            ...
            if (Calculations.isBlockCulled(block)) {
                // Track that we suppressed an update so we can force-sync state later
            ...
            try {
                kaiakk.powerhouse.async.BlockKey bk = new kaiakk.powerhouse.async.BlockKey(block.getX(), block.getY(), block.getZ(), block.getWorld().getName());
        ...
        public void run() {
            SchedulerHelper.runAsync(plugin, new Runnable() {
                public void run() {
                    ...
                    try {
                        ...
                        if (mspt > 0.0 && ScaleUtils.continuousScaleFromMspt(mspt) > recoveredScale) {
                            try {
                                ...
                                kaiakk.powerhouse.sync.DebugLog.debug("Powerhouse: redstone safety valve - cleared all culled locations as MSPT recovered (" + String.format("%.1f", mspt) + "mspt)");
                            } catch (Throwable ignored) {}
                        }
                    } catch (Throwable ignored) {}
                    ...
                    try {
                        java.util.Map<kaiakk.powerhouse.async.BlockKey, Integer> drained = new java.util.HashMap<>();
                        for (java.util.Map.Entry<kaiakk.powerhouse.async.BlockKey, java.util.concurrent.atomic.AtomicInteger> e : redstoneListenerCounts.entrySet()) {
                            try {
                        ...
                        if (!drained.isEmpty()) {
                            try { kaiakk.powerhouse.async.Calculations.applyListenerCounts(drained); } catch (Throwable ignored) {}
                        }
                    } catch (Throwable ignored) {}
    ...
    kaiakk.powerhouse.sync.DebugLog.debug("Adaptive redstone culling active!");
}
                            ...
                            SchedulerHelper.run(plugin, new Runnable() {
                                public void run() {
                                    for (Location loc : expired) {
                                        ...
                                        String locStr = formatLocation(loc);
                                        kaiakk.powerhouse.sync.DebugLog.debug("Restored redstone at: " + locStr);
    ...
    private void startArrowCleanupTask(double intervalSeconds) {
        ...
        arrowTask = SchedulerHelper.runTimerSeconds(plugin, new Runnable() {
            public void run() {
                ...
                if (tname != null && (kaiakk.powerhouse.sync.FoliaChecker.isFolia(plugin) || tname.contains("Region") || tname.contains("Threaded"))) {
    ...
    private void attemptPruneTileEntities() {
        SchedulerHelper.runLater(plugin, new Runnable() {
            public void run() {
                ...
                if (removed > 0) {
                    kaiakk.powerhouse.sync.DebugLog.debug("Pruned " + removed + " sign/skull tile-entities from ticking lists (best-effort)");
                }
            }
        }, 20L);
    }
        ...
        try {
            ...
            if (handle != null) {
                try {
                    ...
                    die.invoke(handle);
                    kaiakk.powerhouse.sync.DebugLog.debug("markEntityDead via die(): " + ent.getType() + " @ " + ent.getLocation());
                    return;
                } catch (Throwable ignored) {}
                ...
                try {
                    ...
                    try { kaiakk.powerhouse.sync.DebugLog.debug("markEntityDead via setDead(): " + ent.getType() + " @ " + ent.getLocation()); } catch (Throwable ignored) {}
                    return;
                } catch (Throwable ignored) {}
                ...
                try {
                    ...
                    try { kaiakk.powerhouse.sync.DebugLog.debug("markEntityDead via dead field: " + ent.getType() + " @ " + ent.getLocation()); } catch (Throwable ignored) {}
                    return;
                } catch (Throwable ignored) {}
