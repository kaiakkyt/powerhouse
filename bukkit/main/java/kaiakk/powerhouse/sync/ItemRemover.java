...
public class ItemRemover implements Listener {
            ...
            if (!nearby) {
                try {
                    ...
                    if (!recentPlayer && !recentDispense) {
                        ...
                        kaiakk.powerhouse.sync.DebugLog.debug("ItemRemover: immediate removed " + stack.getType() + " @ " + it.getLocation());
                    } else {
                        kaiakk.powerhouse.sync.DebugLog.debug("ItemRemover: skipped recent player/dispense item " + stack.getType() + " @ " + it.getLocation());
                    }
                } catch (Throwable ignored) {}
            }
        } catch (Throwable ignored) {}
                        ...
                        for (org.bukkit.entity.Entity e : chunk.getEntities()) {
                            ...
                            if (!nearby) {
                                try {
                                    ...
                                    if (!recentPlayer && !recentDispense) {
                                        AllOptimizations.getInstance().markEntityDead(it); itemRemovalsLocal++; kaiakk.powerhouse.sync.DebugLog.debug("ItemRemover: purge removed " + stack.getType() + " @ " + it.getLocation());
                                    } else {
                                        kaiakk.powerhouse.sync.DebugLog.debug("ItemRemover: purge skipped recent item " + stack.getType() + " @ " + it.getLocation());
                                    }
                                } catch (Throwable ignored) {}
                            }
                        }
    ...
    public void reloadSettings() {
        try {
            double d = kaiakk.powerhouse.ConfigHelp.getDouble("item-hiding.distance", 16.0);
            double v = kaiakk.powerhouse.ConfigHelp.getDouble("item-hiding.velocity-threshold", 5.0);
            this.hideDistance = d;
