...
import kaiakk.powerhouse.async.BookCalculator;
...
public class BookLagger implements Listener {
    private static BookLagger INSTANCE = null;
    private final Plugin plugin;

    private BookLagger(Plugin plugin) {
        this.plugin = plugin;
    }

    public static synchronized void init(Plugin plugin) {
        if (INSTANCE == null) {
            INSTANCE = new BookLagger(plugin);
    ...
    private boolean sanitizeIfNeeded(ItemStack stack) {
        try {
            ...
            if (sanitized != null) {
                ...
                try { kaiakk.powerhouse.sync.DebugLog.debug("BookLagger: sanitized a malicious book"); } catch (Throwable ignored) {}
                return true;
            }
