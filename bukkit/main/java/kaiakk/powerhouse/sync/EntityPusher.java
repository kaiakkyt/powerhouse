...
import kaiakk.powerhouse.async.CrammingCalculator;
...
public class EntityPusher implements Scalable {
    ...
    private void setAiSafe(LivingEntity ent, boolean enabled) {
        try { kaiakk.powerhouse.sync.ApiCompat.setAI(ent, enabled); } catch (Throwable ignored) {}
    }
    ...
    private void tickMain() {
        ...
        final List<EntitySnapshot> snapCopy = new ArrayList<>(globalSnaps);
            ...
            if (toRemove == null || toRemove.isEmpty()) return;
                    try { kaiakk.powerhouse.sync.DebugLog.debug("EntityPusher: cramming detection found " + (toRemove == null ? 0 : toRemove.size()) + " candidates"); } catch (Throwable ignored) {}
            ...
            SchedulerHelper.runLater(plugin, () -> {
                ...
                try {
                    ...
                    if (ao != null) {
                        ...
                        if (removed > 0) {
                            try { kaiakk.powerhouse.sync.DebugLog.debug("EntityPusher: removed " + removed + " entities due to cramming (threshold=" + threshold + ", radius=" + radius + ")"); } catch (Throwable ignored) {}
                        }
                    }
                } catch (Throwable ignored) {}
            }, 1L);
